...
impl<E: Evaluator> MiniMaxBot<E> {
    pub(crate) fn new(depth: u8, evaluator: E) -> Self {
        MiniMaxBot { depth, evaluator }
    }
}

impl<E: Evaluator> Player for MiniMaxBot<E> {
    fn get_move(&mut self, board: &Board, is_white: bool) -> Move {
        ...
        for mv in moves {
            ...
            let eval = self.get_eval(&mut my_board, !is_white, self.depth - 1);
            #[cfg(debug_assertions)]
        ...
        #[cfg(debug_assertions)]
        {
            ...
            eprintln!(
                "--- minimax ({}) depth={} white_dist={} black_dist={} nodes={} ---",
...
impl<E: Evaluator> MiniMaxBot<E> {
    fn get_eval(&self, board: &mut Board, is_white: bool, depth: u8) -> i32 {
        #[cfg(debug_assertions)]
        ...
        for mv in moves {
            ...
            let eval = self.get_eval(board, !is_white, depth - 1);
